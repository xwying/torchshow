...
|The image tensor has been normalized so matplotlib cannot display it correctly. | TensorShow does the conversion automatically.|

## Visualizing Mask Annotations
For projects related to Semantic Segmentation or Instance Segmentation, we usually need to visualize the mask tensor -- either from ground truth or from model's prediction. This can be easily done using TensorShow.
...
TensorShow visualizes video clips as an `matplotlib.func_animation` object and may not be able to directly displayed in a notebook. The following example shows a simple trick to display it.
